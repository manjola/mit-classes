...
class RectangularRoom(object):
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        current_dirt = self.tiles[(x,y)]
        new_dirt = current_dirt - capacity
        if new_dirt>=0:
            self.tiles[(x,y)] = new_dirt
        else:
            self.tiles[(x,y)] = 0
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_valid(self, pos):
        ...
        """
        if self.is_position_in_room(pos)== True:
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for tr in range(num_trials):
        ...
        anim = ps3_visualize.RobotVisualization(num_robots, width, height, room.furniture_tiles, 0.2)
        total_tiles = room.get_num_tiles()
        robots = [robot_type(room,speed,capacity) for i in range(num_robots)]
        steps = 0
        cleaned_tiles = 0
        while float(cleaned_tiles/total_tiles) < min_coverage:
            for r in robots:
        ...
        steps_total+= steps
        anim.done()
    return steps_total/num_trials

#print ('avg time steps: ' + str(run_simulation(1, 1.0, 1, 5, 5, 3, 1.0, 50, StandardRobot)))
#print ('avg time steps: ' + str(run_simulation(1, 1.0, 1, 10, 10, 3, 0.8, 50, StandardRobot)))
#print ('avg time steps: ' + str(run_simulation(1, 1.0, 1, 10, 10, 3, 0.9, 50, StandardRobot)))
#print ('avg time steps: ' + str(run_simulation(1, 1.0, 1, 20, 20, 3, 0.5, 50, StandardRobot)))
print ('avg time steps: ' + str(run_simulation(7, 1.0, 1, 20, 20, 3, 0.5, 50, StandardRobot)))
